...
export function LeadTable({ leads, onStatus }: { leads: Lead[]; onStatus: (lead: Lead, status: Lead["status"]) => void }) {
  return (
    <div className="lead-table-wrap">
      <div className="lead-table-head"><div><h3>Opportunity stream</h3><p>Prioritised by live intent and recency.</p></div><button>View all <ArrowUpRight size={15} /></button></div>
          ...
          <tbody>
            {leads.map((lead) => (
              <tr key={lead.id}>
                ...
                <td><button className="row-menu"><MoreHorizontal size={18} /></button></td>
              </tr>
            ))}
          </tbody>
        </table>
